...
import static jsat.math.MathTricks.sqr;
...
public class SparseVector extends Vec {
    ...
    public SparseVector(Vec toCopy) {
        ...
        for (IndexValue iv : toCopy) {
            indexes[used] = iv.index;
            values[used++] = iv.value;
        }
    }
    ...
    public void set(int index, double val) {
        ...
        if (insertLocation >= 0) {
            if (val != 0) {//set it {
                if ((Math.abs(val - values[insertLocation])) < Double.MIN_NORMAL)
                    return; //no change
    ...
    public double skewness() {
        ...
        for (int i = 0; i < used; i++) {
            double vi = values[i] - mean;
            numer += MathTricks.cube(vi);
            denom += MathTricks.sqr(vi);
        }

        //All the zero's we arent storing
        numer += MathTricks.cube(-mean) * (length - used);
        denom += MathTricks.sqr(-mean) * (length - used);
    ...
    public double kurtosis() {
        ...
        return tmp / MathTricks.sqr(var) - 3;
    }
        ...
        if (Math.abs(c) > Double.MIN_NORMAL) {
            ...
            if (v instanceof SparseVector) {
                ...
                while (p1 < used && iv != null) {
                    ...
                    if (a1 == a2) {
                        values[p1++] += c * iv.value;
                        if (iter.hasNext())
                            iv = iter.next();
                        else
                            break;
                    } else if (a1 > a2) {
                        this.set(a2, c * iv.value);
                        p1++;
                        if (iter.hasNext())
                            iv = iter.next();
                        else
    ...
    public Iterator<IndexValue> getNonZeroIterator(final int start) {
        ...
        Iterator<IndexValue> itor = new Iterator<IndexValue>() {
            ...
            public IndexValue next() {
                indexValue.index = indexes[curUsedPos];
                indexValue.value = values[curUsedPos++];
                return indexValue;
            }
